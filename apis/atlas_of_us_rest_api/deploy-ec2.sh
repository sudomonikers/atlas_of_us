# EC2 Deployment Script for Atlas of Us API (Rust)
# This script deploys the dockerized Rust application to an EC2 instance
...
REMOTE_DIR="/home/$EC2_USER/atlas_of_us_rest_api"

echo "Deploying Atlas of Us Rust API to EC2..."

# Build Docker image locally for x86_64 platform (EC2 architecture)
echo "Building Docker image locally for x86_64 platform..."
docker build --platform linux/amd64 -t atlas-of-us-api:latest .

# Save image to tar file
echo "Saving Docker image to file..."
docker save atlas-of-us-api:latest > atlas-of-us-api.tar
...
# Copy image and docker-compose to EC2
echo "Copying Docker image and config to EC2..."
...
ssh -i "$EC2_KEY_PATH" -o StrictHostKeyChecking=no "$EC2_USER@$EC2_HOST" << EOF
    ...
    sudo docker load < atlas-of-us-api.tar
